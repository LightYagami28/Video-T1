...
def best_of_N(
    ...
    QUESTION_PATH = "./score_questions.txt"
    WEIGHT_PATH = "./questions_weight.json"
    ...
    # 这个格式和输出格式是对应的,最后的结果是 final_*.mp4,中间输出是path_*.mp4
...
def judging(
    ...
    QUESTION_0_PATH = "./question0.txt"
    QUESTION_1_PATH = "./question1.txt"
    QUESTION_2_PATH = "./question2.txt"
    ...
    for encoding in encodings:
        path = os.path.join(output_dir, f"path_{encoding}.mp4")
    ...
    if judge_mode == "pf":
        # PF模式选择,如果全部问题回答都是yes就通过,否则不通过
        # 但是由于实际情况下较少全部通过的,所以引入half_accepted
