#!/usr/bin/env python
"""
Video-T1 Video Generation Script.
"""
...
# Base Model(Pyramid-Flow)
MODEL_DTYPE = 'bf16'  # Data type for model weights (bf16 for mixed precision)
...
# Result and intermediate files path
...
def init_pyramid_model(model_path, device, model_variant="diffusion_transformer_384p", model_name="pyramid_flux"):
    ...
    model = PyramidDiTForVideoGeneration(
        ...
        frame_selection_callback=True,  # Enable frame selection callback (purpose needs clarification if open-sourced)
        video_sync_group=8,            # Video sync group size (purpose needs clarification if open-sourced)
    )
...
def main():
    ...
    # --- Device Setup ---
    torch.cuda.set_device(args.device_id) # Set CUDA device to be used
    device = f"cuda:{args.device_id}" if torch.cuda.is_available() else "cpu" # Determine device string
    ...
    best_video = generator.videot1_gen_wo_imgcot(
        prompt=prompt,
